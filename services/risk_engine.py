...
def build_trade(signal_data, price, base_leverage=5):
    ...
    # Используем total_score (уже умноженный на 100) для расчета тейка
    total_score = abs(signal_data.get("total_score", signal_data.get("score", 0)))
    volatility = signal_data.get("volatility", 0)

    leverage = min(max(3, base_leverage), 7)

    stop_buffer = 0.004 + volatility / 7000
    take_buffer = 0.012 + total_score * 0.001   # так было изначально
